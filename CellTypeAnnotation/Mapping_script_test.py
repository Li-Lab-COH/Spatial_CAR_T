...
"""
Cell2location spatial mapping — PARAMETER SWEEP / TEST VERSION.

Identical to Mapping_script.py except that output paths are namespaced by a
run_tag built from the key hyperparameters, so every parameter combination
writes to its own directory and file without overwriting previous runs.

Output layout example for --detection-alpha 200 --N-cells-per-location 1.5
  --epochs 200 --export-num-samples 200 --export-batch-size 1024:

  Model:       <proj>/results/cell2location/c2l_lvl2_a200_n1.5_ep200_s200_bs1024/
  Predictions: <proj>/results/cell2location/C2L_inputs/predicted_cells_lvl2_a200_n1.5_ep200_s200_bs1024.h5ad
  Figures:     <proj>/figures/cell2location/mapping/lvl2_a200_n1.5_ep200_s200_bs1024/

Do NOT edit Mapping_script.py for sweep experiments — use this file instead.
"""
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Run cell2location spatial mapping (test/sweep version).")
...
def main() -> None:
    ...
    # --- Run tag: unique identifier for this parameter combination ---
    # Format: a<alpha>_n<ncells>_ep<epochs>_s<samples>_bs<batch>
    # Example: a200_n1.5_ep200_s200_bs1024
    run_tag = (
        f"a{args.detection_alpha:g}"
        f"_n{args.N_cells_per_location:g}"
        f"_ep{args.epochs}"
        f"_s{args.export_num_samples}"
        f"_bs{args.export_batch_size}"
    )
    print(f"[{now_str()}] Run tag: {run_tag}")

    # --- Paths (unique per parameter combination) ---
    proj_folder          = Path(args.proj_folder)
    c2l_folder           = proj_folder / "results/cell2location"
    input_folder         = c2l_folder / "C2L_inputs"
    figures_folder       = proj_folder / "figures/cell2location/mapping" / f"{lvl_name_short}_{run_tag}"
    run_name             = c2l_folder / f"c2l_{lvl_name_short}_{run_tag}"
    spatial_loc          = input_folder / args.spatial_file
    trained_sc_data_file = input_folder / f"sc_with_signatures_{lvl_name_short}.h5ad"
    predicted_cells_file = input_folder / f"predicted_cells_{lvl_name_short}_{run_tag}.h5ad"
    ...
    out = figures_folder / f"training_history_{args.epochs}epochs.png"
    ...
    try:
        n = call_and_save_plots(
            ...
            figures_folder,
            f"spatial_QC_{args.epochs}epochs",
        )
